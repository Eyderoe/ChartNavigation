...
std::pair<double, double> PdfView::trans () {
    ...
    qDebug() << std::format("raw ({:.1f},{:.1f})", x, y);
    // 获取当前渲染范围


    // Still INOP
    double scaleFactor = this->zoomFactor();
    ...
    double startX = (double)scrollX / scaleFactor;
    double startY = (double)scrollY / scaleFactor;
    double endX = (double)(scrollX + viewportWidth) / scaleFactor;
    double endY = (double)(scrollY + viewportHeight) / scaleFactor;





    qDebug() << std::format("view ({},{})", viewportWidth, viewportHeight);
    qDebug() << std::format("from ({:.2f},{:.2f}) to ({:.2f},{:.2f})", startX, startY, endX, endY);
    // 计算
    x = (x - startX) / (endX - startX) * viewportWidth;
    y = (y - startY )/ (endY - startY) * viewportHeight;
    qDebug() << std::format("in view ({:.1f},{:.1f})", x, y);
    qDebug() << "---------------";
    return {x, y};
}
